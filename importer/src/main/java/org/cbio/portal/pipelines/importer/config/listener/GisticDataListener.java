 ...
 * @author ochoaa
 */
public class GisticDataListener implements StepExecutionListener {

    private static final Log LOG = LogFactory.getLog(GisticDataListener.class);
    
    @Override
    public void beforeStep(StepExecution stepExecution) {        
        // add cancer study, datafile, and gistic file metadata from tasklet to step execution context
        ...
        stepExecution.getExecutionContext().put("gisticMetadata", stepExecution.getJobExecution().getExecutionContext().get("gisticMetadata"));
    }
    ...
    public ExitStatus afterStep(StepExecution stepExecution) {
        ...
        // get number of entries skipped, total genes loaded, and total records imported
        ...
        int gisticDataCount = (int) stepExecution.getExecutionContext().get("gisticDataCount");
        int gisticGeneDataCount = (int) stepExecution.getExecutionContext().get("gisticGeneDataCount");
        
        if ((gisticDataCount + gisticGeneDataCount) == 0) {
            LOG.error("No records were imported into GISTIC or GISTIC_TO_GENE");
            ...
            LOG.info("Total records imported into GISTIC: " + gisticDataCount);
            LOG.info("Total records imported into GISTIC_TO_GENE: " + gisticGeneDataCount);
        }
