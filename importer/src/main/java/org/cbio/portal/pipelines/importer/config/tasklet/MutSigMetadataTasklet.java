 ...
 * @author ochoaa
 */
public class MutSigMetadataTasklet implements Tasklet {
    ...
    public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {        
        ...
        // load mutsig metadata from datafile
        MultiKeyMap mutSigMetadata = DataFileUtils.loadDataFileMetadata(dataFile);
        
        // datafile and mutsig metadata to execution context
        chunkContext.getStepContext().getStepExecution().getJobExecution().getExecutionContext().put("dataFile", dataFile);
        chunkContext.getStepContext().getStepExecution().getJobExecution().getExecutionContext().put("mutSigMetadata", mutSigMetadata);
